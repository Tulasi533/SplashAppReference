...
import 'package:splash_app/AdminFlow/ManageFaculty/UpdateCancelFaculty.dart';
...
class UpdateFacultyPage extends StatefulWidget {
  const UpdateFacultyPage({ Key? key }) : super(key: key);

  @override
  State<UpdateFacultyPage> createState() => _UpdateFacultyPageState();
}
...
class _UpdateFacultyPageState extends State<UpdateFacultyPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        elevation: 0,
        title: Text("Search Faculty to Update"),
        centerTitle: true,
        leading: IconButton(
                  ...
                  child:ListTile(
                    onTap: () {
                      Navigator.push(
                        context, 
                        MaterialPageRoute(
                          builder: (context) => UpdateCancelFaculty(facultyId: d.facultyid)
                        )
                      );
                    },
